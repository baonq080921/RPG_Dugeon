...
- Use with caution - can lead to tight coupling and testing difficulties
- Consider dependency injection or event systems instead
- If using singletons using the  Assets/Base/Singleton.cs pattern, ensure proper cleanup and null checks

### Object Pooling
- Use for frequently instantiated/destroyed objects (bullets, enemies,etc...)
- when create a pool use ObjectPool<T> from UnityEngine.Pool namespace
- Ensure proper reset logic when reusing objects
